...
public class LibraryThrows {

	private List<Book> books;
	private Scanner sc;
	
	public LibraryThrows() {
	...
	// RuntimeException은 Unchecked-Exception이므로 throws RuntimeException을 생략할 수 있다.
	private void deleteBook() throws RuntimeException {
		...
		if(isbn.isEmpty() == false) {
			// 1. 객체 기반 삭제(Book 객체 간 비교가 필요하기 때문에 equals를 오버라이드 해야 한다.
			for(Book book : books) {
